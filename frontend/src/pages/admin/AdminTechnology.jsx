...
const STORAGE_KEY = "admin_technology_draft_v1";
...
function loadInitialStories() {
  if (typeof window === "undefined") return cloneStories(technologyArticles);

  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return cloneStories(technologyArticles);
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed) || parsed.length === 0) return cloneStories(technologyArticles);
    return cloneStories(parsed);
  } catch {
    return cloneStories(technologyArticles);
  }
}
...
export default function AdminTechnology() {
    ...
    async function loadPublished() {
      ...
      try {
        ...
        if (mounted && Array.isArray(json) && json.length > 0) {
          ...
          setNotice("Loaded published technology stories.");
        }
      } catch {
  ...
  function addStory() {
    ...
    setEditorOpen(true);
    setNotice("New technology story added.");
  }
  ...
  function saveDraft() {
    try {
      ...
      setNotice("Technology draft saved locally.");
    } catch {
  ...
  function resetFromDefault() {
    const next = cloneStories(technologyArticles);
    setStories(next);
    setSelectedId(next[0]?.id || "");
    setNotice("Reset to default technology content.");
  }
  ...
  async function publishStories() {
    ...
    try {
      ...
      setStories(next);
      setSelectedId(next[0]?.id || "");
      setNotice("Published. Technology page data is now updated.");
    } catch (err) {
  ...
  async function uploadSelectedImage(file) {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/uploads/technology`, {
        method: "POST",
    ...
    <section className="space-y-4 text-zinc-900">
      <div className="rounded-2xl border border-zinc-300 bg-white/70 p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-[0.04em] md:text-4xl">Technology Edits</h1>
            <p className="mt-1 text-sm text-zinc-600">
              Match the live Technology page order and edit story cards before publishing from code.
            </p>
          </div>
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-[1.2fr_1fr]">
            <article className="rounded-2xl border border-zinc-300 bg-zinc-100 p-3">
              <div className="overflow-hidden rounded-xl bg-zinc-200">
                {featured.image ? (
                  <img src={resolveImageUrl(featured.image)} alt={featured.title || "Featured technology story"} className="h-72 w-full object-cover" />
                ) : (
              ...
              <p className="pt-3 text-xs uppercase tracking-[0.12em] text-zinc-600">
                {featured.category || "Technology"} - {featured.date || "Date"}
              </p>
              ...
              {spotlight.map((story) => (
                <article key={story.id} className="rounded-2xl border border-zinc-300 bg-zinc-100 p-3">
                  <div className="grid gap-3 sm:grid-cols-[140px_1fr]">
                    <div className="overflow-hidden rounded-xl bg-zinc-200">
                      {story.image ? (
                        <img src={resolveImageUrl(story.image)} alt={story.title || "Technology story"} className="h-28 w-full object-cover" />
                      ) : (
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {feed.map((story) => (
              <article key={story.id} className="rounded-2xl border border-zinc-300 bg-zinc-100 p-3">
                <div className="overflow-hidden rounded-xl bg-zinc-200">
                  {story.image ? (
                    <img src={resolveImageUrl(story.image)} alt={story.title || "Technology story"} className="h-40 w-full object-cover" />
                  ) : (
                ...
                <p className="pt-3 text-xs uppercase tracking-[0.12em] text-zinc-500">
                  {story.category || "Technology"} - {story.date || "Date"}
                </p>
                ...
                {!selected ? (
                  ...
                  <>
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Link
                        to={`/technology/article/${selected.id}`}
                        className="rounded-lg border border-zinc-300 bg-white px-3 py-2 text-xs font-semibold text-zinc-800 hover:bg-zinc-100"
                      >
                        Open Article
                      </Link>
