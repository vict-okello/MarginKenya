    ...
    >
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className={`mt-2 text-sm ${isDark ? "text-white/70" : "text-zinc-600"}`}>
              Site-aligned dashboard with live metrics and quick editorial controls.
            </p>
      ...
      <section data-anim className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <KpiCard
          isDark={isDark}
          label="Avg Read Time"
          value={formatTime(data.kpis.avgReadTimeSec)}
              ...
              >
                <div className="flex items-center justify-between gap-3 text-sm">
                  <p className="min-w-0 truncate">
                    <span className={isDark ? "text-white/45 mr-2" : "text-zinc-500 mr-2"}>
                      #{index + 1}
                    </span>
                    {article.title}
                  </p>
...
// Make pill respect theme (optional)
function Pill({ label }) {
  return (
