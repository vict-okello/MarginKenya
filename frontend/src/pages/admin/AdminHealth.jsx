...
const STORAGE_KEY = "admin_health_draft_v1";
...
function loadInitialStories() {
  if (typeof window === "undefined") return cloneStories(healthArticles);

  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return cloneStories(healthArticles);
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed) || parsed.length === 0) return cloneStories(healthArticles);
    return cloneStories(parsed);
  } catch {
    return cloneStories(healthArticles);
  }
}
...
export default function AdminHealth() {
    ...
    async function loadPublished() {
      ...
      try {
        ...
        if (mounted && Array.isArray(json) && json.length > 0) {
          ...
          setNotice("Loaded published health stories.");
        }
      } catch {
  ...
  function saveDraft() {
    try {
      ...
      setNotice("Health draft saved locally.");
    } catch {
  ...
  function resetFromDefault() {
    const next = cloneStories(healthArticles);
    setStories(next);
    setSelectedId(next[0]?.id || "");
    setNotice("Reset to default health content.");
  }
  ...
  async function uploadSelectedImage(file) {
    ...
    try {
      ...
      const res = await fetch(`${API}/api/uploads/health`, {
        method: "POST",
  ...
  async function publishStories() {
    ...
    try {
      ...
      setStories(next);
      setSelectedId(next[0]?.id || "");
      setNotice("Published. Health page is now updated.");
    } catch (err) {
    ...
    <section className="space-y-4 text-zinc-900">
      <div className="rounded-2xl border border-zinc-300 bg-white/70 p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <h1 className="text-3xl font-black uppercase tracking-[0.04em] md:text-4xl">Health Edits</h1>
            <p className="mt-1 text-sm text-zinc-600">
              Match the live Health page layout, edit stories, and publish updates.
            </p>
          </div>
          ...
          <div className="mt-4 grid gap-4 lg:grid-cols-[1.25fr_1fr]">
            <article className="rounded-2xl border border-zinc-300 bg-zinc-100 p-3">
              <div className="overflow-hidden rounded-xl bg-zinc-200">
                {lead.image ? (
                  <img src={resolveImageUrl(lead.image)} alt={lead.title || "Lead health story"} className="h-64 w-full object-cover" />
                ) : (
                  <div className="flex h-64 items-center justify-center text-sm text-zinc-500">No image</div>
                )}
              </div>
              <p className="pt-3 text-xs uppercase tracking-[0.12em] text-zinc-600">Lead Story - {lead.date || "Date"}</p>
              ...
              {secondary.map((story) => (
                <article key={story.id} className="rounded-2xl border border-zinc-300 bg-zinc-100 p-3">
                  <div className="grid gap-3 sm:grid-cols-[140px_1fr]">
                    <div className="overflow-hidden rounded-xl bg-zinc-200">
                      {story.image ? (
                        <img src={resolveImageUrl(story.image)} alt={story.title || "Health story"} className="h-28 w-full object-cover" />
                      ) : (
                        <div className="flex h-28 items-center justify-center text-xs text-zinc-500">No image</div>
                      )}
                    </div>
                    <div>
                      <p className="text-xs uppercase tracking-[0.12em] text-zinc-600">{story.date || "Date"}</p>
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {rest.map((story) => (
              <article key={story.id} className="rounded-2xl border border-zinc-300 bg-zinc-100 p-3">
                <div className="overflow-hidden rounded-xl bg-zinc-200">
                  {story.image ? (
                    <img src={resolveImageUrl(story.image)} alt={story.title || "Health story"} className="h-36 w-full object-cover" />
                  ) : (
